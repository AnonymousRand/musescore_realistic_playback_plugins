...
MuseScore {
      ...
      function applyChanges() {
            ...
            // apply velocity offset to all dynamics in selection
            curScore.startCmd(); // required for operation to be undoable
            for (var i = 0; i < elements.length; i++) {
                  var element = elements[i];
                  if (element.type === Element.DYNAMIC) {
                        element.velocity += offset;
                        // clamp velocities to -127 to 127,
                        // which is the max allowed by MuseScore offset velocity
                        if (element.velocity > 127) {
                              element.velocity = 127;
                        } else if (element.velocity < -127) {
                              element.velocity = -127;
                        }
                  }
            }
